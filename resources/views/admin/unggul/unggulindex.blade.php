...
<div class="container-fluid">
    <div class="row">
        <div class="col-12">
            <div class="card"> 
                <div class="card-body">
                    <a href="{{route('unggul.create')}}" class=" btn-priamry btn-sm">
                        ...
                        <table id="zero_config" class="table table-striped table-bordered">
                            ...
                            <tbody>
                                @foreach ($data as $item =>$value)
                                    <tr>
                                        <Td>{{ $item+1 }}</Td>
                                        <Td>{!! $value->keunggulan!!}</Td>
                                        <Td>
                                            {{-- <a href="{{ url('product/'.$value->id.'/edit')}}" class=" btn-priamry btn-sm" > --}}
                                            <a href="{{route('unggul.edit', $value->id)}}" class=" btn-priamry btn-sm">
                                                <button class="btn btn-primary btn-sm" >Ubah</button>
                                            </a>|

                                            <form action="{{ route('unggul.destroy', $value->id) }}"  method="POST" class="d-inline">
                                                @csrf
                                                @method('delete')
